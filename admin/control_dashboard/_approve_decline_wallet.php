...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['txn_id'])) {
        ...
        $query = "UPDATE user_wallet SET wallet_txn_status = ? WHERE user_wallet_id = ? AND wallet_txn_status = 'P'";
        if ($stmt = CONN->prepare($query)) {
            $stmt->bind_param('ss',$actionId,$userWalletId); // Bind the wallet ID parameter
            $stmt->execute();
