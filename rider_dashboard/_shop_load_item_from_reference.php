...
include_once "../client/_class_Bookings.php";
...
try {
   ...
   // Assuming USER_LOGGED is defined as the logged-in user ID

    // Use prepared statements to avoid SQL injection
 


    $sql_shop_order_ref = "SELECT * FROM `shop_item_merchant_view` WHERE `shop_order_ref_num` = ?";
    $stmt = CONN->prepare($sql_shop_order_ref);
    if (!$stmt) {
        throw new Exception("Failed to prepare statement: " . $conn->error);
    }

    $stmt->bind_param("s", $ref_num); // Bind parameters
    $stmt->execute();
