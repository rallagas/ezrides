...
/**
 * Returns a bounds object to restrict location search based on a central point and a radius.
 * 
 * @param {number} latitude - The latitude of the center.
 * @param {number} longitude - The longitude of the center.
 * @param {number} radius - The radius to restrict the search (in meters).
 * @returns {google.maps.LatLngBounds} - The bounds object to be used in the autocomplete request.
 */
function getBiasBounds(latitude = 0, longitude = 0, radius = 50000) {
  const center = new google.maps.LatLng(latitude, longitude);
  const bounds = new google.maps.LatLngBounds();

  // Extend the bounds with a point that's 'radius' meters away from the center
