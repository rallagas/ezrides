...
FROM
    (
        (
            ...
            LEFT JOIN `users` `u`
            ON
                (`ab`.`user_id` = `u`.`user_id`)
            )
        LEFT JOIN `users` `ur`
        ON
            (
                `ab`.`angkas_rider_user_id` = `ur`.`user_id` AND `ur`.`t_rider_status` = 1
            )
        )
...
SELECT
    ...
    MAX(
        DATE_FORMAT(`so`.`order_date`, '%m-%d-%Y %H:%i')
    ) AS `order_date`,
    MAX(`so`.`payment_status`) AS `shop_payment_status`,
    CASE WHEN TIMESTAMPDIFF(
        SECOND,
        `so`.`order_date`,
        CURRENT_TIMESTAMP()) < 60 THEN CONCAT(
            TIMESTAMPDIFF(
                SECOND,
                `so`.`order_date`,
                CURRENT_TIMESTAMP()),
                ...
                CASE WHEN TIMESTAMPDIFF(
                    SECOND,
                    `so`.`order_date`,
                    CURRENT_TIMESTAMP()) > 1 THEN 's' ELSE ''
                END,
                ' ago'
            ) WHEN TIMESTAMPDIFF(
                MINUTE,
                ...
                CURRENT_TIMESTAMP()) < 60 THEN CONCAT(
                    TIMESTAMPDIFF(
                        ...
                        HOUR,
                        `so`.`order_date`,
                        CURRENT_TIMESTAMP()) < 24 THEN CONCAT(
                            TIMESTAMPDIFF(
                                HOUR,
                                `so`.`order_date`,
                                CURRENT_TIMESTAMP()),
                                ...
                                DAY,
                                `so`.`order_date`,
                                CURRENT_TIMESTAMP()) < 30 THEN CONCAT(
                                    TIMESTAMPDIFF(
                                        DAY,
                                        `so`.`order_date`,
                                        CURRENT_TIMESTAMP()),
                                        ...
                                        MONTH,
                                        `so`.`order_date`,
                                        CURRENT_TIMESTAMP()) < 12 THEN CONCAT(
                                            TIMESTAMPDIFF(
                                                MONTH,
                                                `so`.`order_date`,
                                                CURRENT_TIMESTAMP()),
                                                    ...
                                                    (
                                                        (
                                                            (
                                                                ...
                                                                ON
                                                                    (`so`.`item_id` = `si`.`item_id`)
                                                                )
                                                            LEFT JOIN `shop_merchants` `sm`
                                                            ON
                                                                (
...
SELECT
    ...
    `ab`.`payment_status` AS `payment_status`,
    `ab`.`rating` AS `rating`,
    CASE WHEN `ab`.`payment_status` = 'P' THEN 'Pending Payment' WHEN `ab`.`payment_status` = 'D' THEN 'Payment Declined' WHEN `ab`.`payment_status` = 'C' THEN 'Paid'
...
`rp`.`user_profile_image` AS `rider_profile`,
`rp`.`rider_plate_no` AS `plate_no`,
`rp`.`rider_license_no` AS `license_no`,
`rp`.`vehicle_model_id` AS `vehicle_model`,
`rp`.`vehicle_photo_1` AS `vehicle_photo_1`,
`rp`.`vehicle_photo_2` AS `vehicle_photo_2`,
`rp`.`vehicle_photo_3` AS `vehicle_photo_3`,
`rp`.`vehicle_photo_4` AS `vehicle_photo_4`,
